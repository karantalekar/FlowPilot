...
const primary = (value: unknown, secondary?: unknown) => <div><p className="font-semibold text-slate-900 dark:text-white">{text(value)}</p>{(typeof secondary === 'string' || typeof secondary === 'number') && <p className="mt-0.5 text-[11px] text-slate-400">{String(secondary)}</p>}</div>
...
const configs: Record<string, Config> = {
  ...
  payments: { title: 'Payments', description: 'Review transactions, payment proof and verification status.', resource: 'payments', statuses: ['pending', 'verified', 'rejected'], columns: [
    { key: 'transactionId', label: 'Transaction', render: i => primary(i.transactionId, i.method) }, { key: 'payer.name', label: 'Paid by', render: i => primary(i.payer?.name || 'Unknown', i.payer?.email) }, { key: 'organization.name', label: 'Organization' }, { key: 'plan', label: 'Plan', render: i => <span className="capitalize">{text(i.plan)}</span> }, { key: 'amount', label: 'Amount', render: i => money(i.amount) }, { key: 'status', label: 'Status', render: i => <StatusPill value={i.status} /> }, { key: 'createdAt', label: 'Submitted', render: i => date(i.createdAt) }, { key: 'verifiedAt', label: 'Reviewed', render: i => date(i.verifiedAt) },
  ] },
  'payment-verification': { title: 'Payment verification', description: 'Focused review queue for uploaded payment proof and UTR details.', resource: 'payments', statuses: ['pending'], columns: [
    { key: 'transactionId', label: 'UTR / Transaction ID', render: i => primary(i.transactionId, i.method) }, { key: 'payer.name', label: 'Paid by', render: i => primary(i.payer?.name || 'Unknown', i.payer?.email) }, { key: 'organization.name', label: 'Organization' }, { key: 'plan', label: 'Plan', render: i => <span className="capitalize">{text(i.plan)}</span> }, { key: 'amount', label: 'Amount', render: i => money(i.amount) }, { key: 'status', label: 'Status', render: i => <StatusPill value={i.status} /> }, { key: 'createdAt', label: 'Submitted', render: i => date(i.createdAt) },
  ] },
