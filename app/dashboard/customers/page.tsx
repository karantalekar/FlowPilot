...
export default function CustomersPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.error(getApiError(error, 'Could not save customer'))
    } finally {
      setSaving(false)
    }
  }
  ...
  const remove = async (id: string) => {
    try {
      ...
      setItems(current => current.filter(customer => customer._id !== id))
      toast.success('Customer deleted')
    } catch (error) {
      toast.error(getApiError(error, 'Could not delete customer'))
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            View and manage complete customer details
          </p>
        </div>
          ...
          <CardContent>
            <form onSubmit={submit} className="grid gap-4 md:grid-cols-2">
              ...
              <input
                ...
                onChange={event =>
                  setForm({ ...form, lifetimeValue: Number(event.target.value) })
                }
      ...
      <Card>
        <CardHeader>
          <CardTitle>{items.length} Customers</CardTitle>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {items.map(customer => (
                <div key={customer._id} className="rounded-lg border p-4">
                  ...
                  <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <dd>${(customer.lifetimeValue || 0).toLocaleString()}</dd>
                    </div>
                  </dl>
                </div>
              ))}
      ...
      <ConfirmDialog
        ...
        onOpenChange={open => !open && setCustomerToDelete(undefined)}
        onConfirm={() => customerToDelete && remove(customerToDelete._id)}
      />
    </div>
  )
}
